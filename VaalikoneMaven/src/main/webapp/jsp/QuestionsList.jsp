		...
		<div class="content">
			...
			<ul>
				<c:forEach var="question" items="${requestScope.questionsList}" >
					<li>${question.id}: ${question.question} <a href='/vaalikone/questions/deletequestion?id=${question.id}' onclick='return confirm("Haluatko varmasti poistaa kysymyksen: ${question.question}?");'><button class="button" style="font-size: 20px;">Poista</button></a> <a href='/admin/muokkaaKysymys?id=${question.id}'><button class="button" style="font-size: 20px;">Muokkaa</button></a></li><br>
				</c:forEach>
			</ul>
		</div>
	</body>
